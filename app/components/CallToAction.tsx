...
const actions = [
  { title: "Join as an Ally", desc: "Add your voice and get our monthly newsletter.", cta: "Join as an Ally", action: "openJoinModal", bg: "bg-primary-base", text: "text-white", desc2: "text-white/75", btn: "border border-white/50 text-white hover:bg-white hover:text-primary-base" },
      ...
      <div className="container-site grid grid-cols-1 md:grid-cols-3 gap-6">
        {actions.map((a) => (
          <div key={a.title} className={`${a.bg} rounded-2xl px-9 py-12 flex flex-col min-h-[260px]`}>
            ...
            {a.action === "openJoinModal" ? (
              <button
                onClick={open}
                className={`inline-flex items-center justify-center self-start rounded-full px-6 py-3 font-header font-medium text-sm transition-colors ${a.btn}`}
              >
                {a.cta}
              </button>
            ) : (
